...
namespace QuickQuestionBank.Infrastructure.Services.Repository
{
    ...
    public class QuestionAnswerMappingRepository : IQuestionAnswerMappingRepository
    {
        ...
        public async Task<IReadOnlyList<QuestionAnswerMapping>> GetAllByQuestionIdAsync(Guid QuestionId)
        {
            return await _context.QuestionAnswerMapping.AsNoTracking().Where(x => x.QuestionId == QuestionId).ToListAsync();
        }
    }
}
